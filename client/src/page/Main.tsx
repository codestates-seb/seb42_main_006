...
export default function Main() {
  ...
  const hadleNavMain = () => {
    auth?.isLogin ? navigate('/addpost/create') : navigate('/login');
  };
  ...
  return (
    <MainWrap>
      <MainBox>
        ...
        <MainText>
          우리 서비스는 자신만의 음악, 맛집, 영화 등을 다른 사람들과 공유하고, 취향이 비슷한 사람들끼리 매칭할 수 있는
          서비스입니다.
          <br className="break" />
          취향이 비슷한 사람들과 소통하며 새로운 음악, 맛집, 영화 등을 발견하고 싶으신 분들께, 저희의 서비스를 적극
          추천드립니다!
          <br className="break" />
          우리 서비스는 자신의 취향을 공유하고, 취향이 비슷한 사람들끼리 매칭하여 추천을 제공하는 서비스입니다.{' '}
          <br className="break" />
          "당신의 취향과 어울리는 추천을 놓치지 마세요! 우리 서비스가 당신의 행복을 전달해드립니다."
        </MainText>
      ...
      <MainBox theme={'max'}>
        ...
        <ExList>
          {experienceArr.map((el) => (
            <BoxLi key={el.type} iconType={el.type} label={el.label} text={el.text} />
          ))}
        </ExList>
      </MainBox>
    </MainWrap>
